...
\textbf{Hypothesis:} At least one of SC\_OLD or MiniJudge provides
non-trivial detection beyond naive baselines, and both together cover
failure modes that step-count alone misses.

We score 1,800 chains across five domains using the production
\texttt{llm-guard-kit} scorer and compare against naive baselines.
All results are on held-out chains with verified correctness labels.

\textbf{Important note on chain format:} The SC\_OLD ensemble (AUROC 0.817
from prior work) was validated on chains generated by a full ReAct agent
producing 3--7 step chains. Chains with $\leq 2$ steps carry no loop
or repetition signal; SC\_OLD degrades on such chains.
MiniJudge (which distils Sonnet verdict decisions) is format-independent
and maintains discrimination regardless of chain length.
...
\toprule
\textbf{Domain} & \textbf{n} & \textbf{Wrong\%} & \textbf{Random}
  & \textbf{StepCount} & \textbf{SC\_OLD} & \textbf{MiniJudge} \\
\midrule
HotpotQA (short chains) & 200 & 63\% & 0.500 & 0.620 & 0.557 & \textbf{0.752} \\
TriviaQA & 1000 & 17\% & 0.500 & \textbf{0.720} & 0.709 & 0.610 \\
2WikiMultiHop & 200 & 54\% & 0.500 & 0.551 & \textbf{0.583} & 0.391 \\
MuSiQue & 200 & 73\% & 0.500 & 0.625 & \textbf{0.703} & 0.609 \\
NQ open-domain & 200 & 58\% & 0.500 & 0.547 & \textbf{0.603} & 0.479 \\
\midrule
SC\_OLD original agent (exp77) & 200 & 57\% & 0.500 & $\sim$0.65 & \textbf{0.817} & 0.747 \\
\bottomrule
\end{tabular}%
}
\caption{Baseline comparison across 1,800 chains (five domains).
SC\_OLD beats Random on all domains. SC\_OLD and StepCount are competitive
on TriviaQA ($\Delta = 0.011$), but SC\_OLD dominates on multi-hop domains
(2Wiki, MuSiQue, NQ) where step count carries less signal.
MiniJudge leads on short-chain HotpotQA where behavioral features
have insufficient signal. Bottom row shows SC\_OLD on the original
research-agent chains that produced the 0.817 figure.}
\label{tab:baselines}
\end{table}

\subsection{Experiment 2: Bootstrap CI on Alert Precision (closes Gap 1)}

\textbf{Hypothesis:} AUROC and alert precision estimates are statistically
robust, not sampling artefacts of small held-out sets.

\textbf{Method:} 2,000 bootstrap resamples per dataset. For each resample,
compute AUROC and precision at the threshold $\delta^*$ achieving
FPR $\leq 10\%$. Report as mean with 2.5\%/97.5\% quantile intervals.

\textbf{Results} (2,000 resamples each):
...
\toprule
\textbf{Domain} & \textbf{n} & \textbf{Scorer}
  & \textbf{AUROC (95\% CI)} & \textbf{Prec @ FPR$\leq$10\% (95\% CI)} \\
\midrule
TriviaQA & 1000 & SC\_OLD    & 0.709 [0.660, 0.756] & 0.614 [0.300, 0.909] \\
TriviaQA & 1000 & MiniJudge  & 0.609 [0.563, 0.657] & 0.900 [0.692, 1.000] \\
HotpotQA & 200  & SC\_OLD    & 0.557 [0.475, 0.634] & 0.568 [0.000, 1.000] \\
HotpotQA & 200  & MiniJudge  & 0.752 [0.686, 0.817] & \textbf{1.000 [1.000, 1.000]} \\
2WikiMultiHop & 200 & SC\_OLD & 0.581 [0.502, 0.657] & 0.792 [0.000, 1.000] \\
\bottomrule
\end{tabular}%
}
\caption{Bootstrap CI results (2,000 resamples). MiniJudge achieves
precision 1.000 on HotpotQA at FPR $\leq 10\%$ with a tight CI,
confirming alert-level predictions are reliable when confidence is high.
SC\_OLD precision CIs are wide on small $n$ due to few wrong-chain
examples; this motivates TriviaQA $n=1000$ as the primary evaluation set.}
\label{tab:bootstrap}
\end{table}
...
\subsection{Completed Results (Experiments 1 and 2)}

Experiments 1 and 2 are complete on 1,800 held-out chains.
The key findings are:
\begin{itemize}
  \item SC\_OLD beats Random (0.500) on all five domains.
  \item SC\_OLD leads on multi-hop domains (MuSiQue 0.703, NQ 0.603, 2Wiki 0.581);
        StepCount is competitive only on TriviaQA ($\Delta = 0.011$).
  \item MiniJudge leads on short-chain HotpotQA (0.752 vs 0.557),
        confirming it is format-independent.
  \item MiniJudge precision at FPR $\leq 10\%$ = 1.000 [1.000, 1.000]
        on HotpotQA --- alert-level decisions carry no false positives.
  \item Chain-length sensitivity: SC\_OLD requires $\geq 3$ step chains
        for reliable discrimination. This is the principal scope limitation.
\end{itemize}

\subsection{Pending Figures (Experiments 3--5, GPU required)}
...
evidence for Conjecture~\ref{conj:coverage}.

\textbf{Figure 3 --- Feature-Space Map.}
t-SNE of all confirmed failure traces ($\phi(\tau) \in \mathbb{R}^{12}$),
coloured by taxonomy label, with cluster boundaries. Visually demonstrates
that FARL discovers distinct failure regions, not noise.

\textbf{Figure 4 --- Healer Effectiveness.}
Bar chart of $\Delta_\text{patch}$ per failure mode (before/after fine-tuning).
Paired bar showing $\Delta_\text{acc}$ confirms accuracy is preserved.
Ablation bars (no replay buffer) show catastrophic forgetting.
...
\midrule
\multicolumn{6}{l}{\textit{Prior work validation (original research-agent chains)}} \\
SC\_OLD behavioral & HotpotQA (orig. agent) & 200 & 0.817 & [0.74, 0.89] & \$0 \\
MiniJudge (distilled) & HotpotQA 5-fold CV & 200 & $0.747 \pm 0.10$ & --- & \$0 \\
SC\_OLD + Sonnet (J5) & HotpotQA & 200 & 0.777 & --- & $\sim$\$0.007 \\
SC\_OLD + Sonnet (J5) & TriviaQA & 200 & 0.741 & --- & $\sim$\$0.007 \\
\midrule
\multicolumn{6}{l}{\textit{This work: scored on 1,800 held-out chains (2,000 bootstrap resamples)}} \\
SC\_OLD   & HotpotQA (short chains) & 200  & 0.557 & [0.475, 0.634] & \$0 \\
MiniJudge & HotpotQA (short chains) & 200  & 0.752 & [0.686, 0.817] & \$0 \\
SC\_OLD   & TriviaQA                & 1000 & 0.709 & [0.660, 0.756] & \$0 \\
MiniJudge & TriviaQA                & 1000 & 0.609 & [0.563, 0.657] & \$0 \\
SC\_OLD   & 2WikiMultiHop           & 200  & 0.581 & [0.502, 0.657] & \$0 \\
SC\_OLD   & MuSiQue                 & 200  & 0.703 & --- & \$0 \\
SC\_OLD   & NQ open-domain          & 200  & 0.603 & --- & \$0 \\
MiniJudge Prec @ FPR$\leq$10\% & HotpotQA & 200 & --- & 1.000 [1.000, 1.000] & \$0 \\
SC\_OLD   Prec @ FPR$\leq$10\% & TriviaQA  & 1000 & --- & 0.614 [0.300, 0.909] & \$0 \\
\midrule
\multicolumn{6}{l}{\textit{FARL experiments (requires GPU training --- pending)}} \\
\textbf{FARL hunter} & HotpotQA & TBD & TBD & --- & \$0.0014 \\
\textbf{FARL + LoRA healer} & HotpotQA & TBD & TBD & --- & \$0.0014 \\
\bottomrule
\end{tabular}%
}
\caption{Full results table. Prior work rows are from exp77--exp89 using
the original full ReAct agent. ``This work'' rows are from 1,800 freshly
scored chains using production \texttt{llm-guard-kit}. The gap between
HotpotQA SC\_OLD (0.817 vs 0.557) is explained by chain length: original
chains average 4--6 steps; short-chain HotpotQA averages 2 steps.
MiniJudge Prec $= 1.000$ confirms alert-level decisions are reliable.
FARL rows pending GPU training.}
\label{tab:results}
\end{table}
